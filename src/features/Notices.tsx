    ...
    <>
      {variant === "notice" ? (
        <div className="relative border border-slate-200 rounded-xl">
          <div className="px-4 pt-4 pb-6 flex justify-between">
            <div>{title}</div>
            <div className="text-primary text-xs">{date.split("T")[0]}</div>
          </div>
          <a
            href={`${bucketUrl}/${file}`}
            target="_blank"
          ...
          >
            <Button className="absolute -bottom-6 right-4">Learn More</Button>
          </a>
        </div>
      ) : variant === "job" ? (
            ...
            <div className="flex flex-col gap-2">
              <div className="flex gap-4 justify-center">
                <a
                  ...
                  rel="noopener noreferrer"
                >
                  <button className="secondary-button">View Job</button>
                </a>
                <button
                  className="button"
                  onClick={() => {
      ...
      {isOverlayOpen && (
        <Overlay isOpen={isOverlayOpen} onClose={() => setIsOverlayOpen(false)}>
          <ApplyNow id={jobId} onSuccess={() => setIsOverlayOpen(false)} />
        </Overlay>
      )}
    </>
  );
