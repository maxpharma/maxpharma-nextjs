  ...
  const submitHandler = async (values: any, { resetForm }: any) => {
    ...
    const files = (values.images || []).map((file: any) => {
      ...
      if (file?.base64 && file?.extension) {
        ...
        if (
          ...
          try {
            const url = new URL(file.base64, window.location.origin);
    ...
    try {
      if (updateIdData?.id) {
        ...
        const changed: any = {
          type:
            values.type !== updateIdData.type ? values.type : updateIdData.type,
          name:
        ...
        const prevFiles = (updateIdData.images || []).map((f: any) => {
          if (typeof f === "string" && f.startsWith("uploads/")) return f;
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody>
            ...
            {categoriesRaw.length === 0 && (
              <tr>
                <td colSpan={2} className="px-4 py-2 text-gray-400 italic">
                  No categories found.
                </td>
              </tr>
            )}
      ...
      <div className="bg-white rounded shadow p-4">
        <div className="flex flex-wrap gap-4 mb-4">
          <Dropdown
            ...
            options={["", "Imported Products", "Manufactured Products"]}
            value={filterType}
      ...
      {isProductFormOpen && (
        ...
        >
          <div className="space-y-4 ">
            <h1>{updateIdData?.id ? "Update Product" : "Create Product"}</h1>
            <Formik
                    ...
                    <div className="flex gap-4">
                      <button
                        ...
                        className={`py-2 px-4 rounded ${
                          values.type === "Imported Products"
                            ? "active-button"
                            : "inactive-button"
                        }`}
                        onClick={() => {
                          setFieldValue("type", "Imported Products");
                        }}
                      >
                        Imported Products
                      </button>
                      <button
                        type="button"
                        className={`py-2 px-4 rounded ${
                          values.type === "Manufactured Products"
                            ? "active-button"
                            : "inactive-button"
                        }`}
                        onClick={() => {
                          setFieldValue("type", "Manufactured Products");
                        }}
                      >
