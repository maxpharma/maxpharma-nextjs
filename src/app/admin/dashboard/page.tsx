...
const DashboardPage = () => {
  ...
  const { data: dashboardData } = useSelector((state: any) => state.dashboard);
  ...
  const careerApplyRows = Array.isArray(appliesData)
    ? [...appliesData]
        ...
        .map((item: any) => ({
          id:
            item?.id ??
            `${item?.name ?? "career"}-${item?.phone ?? Math.random()}`,
          type: "Career Apply",
          name: item?.name,
          phone: item?.phone,
  ...
  const productInquiryRows = Array.isArray(inquiriesData)
    ? [...inquiriesData]
        ...
        .map((item: any) => ({
          id:
            item?.id ??
            `${item?.name ?? "inquiry"}-${item?.phone ?? Math.random()}`,
  ...
  const leftTableRows = [...careerApplyRows, ...productInquiryRows].slice(0, 8);
  ...
  const contactRows = Array.isArray(contactsData)
    ? [...contactsData]
        ...
        .map((item: any) => ({
          id:
            item?.id ??
            `${item?.email ?? "contact"}-${item?.phone ?? Math.random()}`,
          name: item?.name,
          phone: item?.phone,
          email: item?.email,
          action: (
